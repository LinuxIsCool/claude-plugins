...
I want to make a subagent in awareness that writes incredible well. Claude code writes very poorly by default. Because
it has been optimized to code, it writes everything like code. I want to start creating agents that write in particular
styles. Actually that could be a really cool plugin. A plugin containing a database of personified writing styles. The
plugin could take the user through organized journeys of generating writing styles, like creating lists of people whose
writing to be like. It's important that the agent spec and the commands and thes skills themselves are written in the styles
of the writers that the user wants to write like because that's how LLMs work, they responds in similar styles to their inputs,
so we want their prompts themselves to already be styled. So the plugin has to ship with a writer agent that can write in
the desired styles to produce the writing style prompts themselves. Perfect example of metaprompting. I also want to make
a plugin called process-dev. As proficient as the feature dev except for creating processes, like the feature dev command
is a process. So I want a command for making commands that is as proficient as the feature dev command at making features. 
I imagine that command taking a super unstructured spec like this very document that I'm writing in right now, and going
through a phases process that builds out the highest leverage command. 
